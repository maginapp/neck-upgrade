...
Vite 使用多入口构建 popup、新标签页和 background worker。自定义插件在构建结束时：

1. 将 `package.json` 的版本写入最终 manifest；
2. 把页面 HTML 从源码层级移动为 `dist/pages/newtab.html` 和 `dist/pages/popup.html`；
3. 把 background 固定输出为 `dist/assets/background.js`；
4. 复制 `_locales` 到产物目录；
5. 由 Vite 复制 `public` 中的图标和数据文件。

Manifest 申请 `storage`、`activeTab` 权限及所有外部数据源的 host permissions。默认语言为英文，并提供英文、简体中文 locale。
