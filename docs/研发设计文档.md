...
设置模型包含：`language`、`theme`、`neck`、`dataType`、`knowledge`、`poetry`。`language` 支持 `zh_CN` 和 `en`，首次使用时根据浏览器语言选择默认值；`poetry` 保存诗词一级分类和二级来源多选结果。其余默认值为系统主题、训练模式、历史内容、维基百科优先和全部诗词来源。设置初始化时会与默认结构合并并校验语言枚举，以兼容缺少新增字段的旧设置。
...
新标签页和 Popup 使用同一个应用级 `LanguageProvider`，根据设置选择英文或简体中文消息目录，并在切换后更新 `document.documentElement.lang`。侧边设置顶部的语言胶囊按钮与主题控件位于同一行；Popup 右上角复用两个快捷控件。所有入口写入同一设置状态，因此 Popup、日期、内容导航、设置和无障碍文案同步重渲染。该运行时机制不依赖固定跟随浏览器语言的 `chrome.i18n.getMessage`；第三方接口和内置古文的原始内容不做机器翻译。
